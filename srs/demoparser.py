...
class Demoparser(object):
	...
	def parsePacket(self, packet):
		...
		if cmd == 1:
			...
			playerName = self.players[playerNum] or ''
			return self.write(locals(), 'cmd', 'msgsize', 'playerNum', 'playerName', 'unitIDCount', 'unitIDs', 'commands')
		elif cmd == 16:
			cmd = 'aishare'
			...
			playerName = self.players[playerNum] or ''
			return self.write(locals(), 'cmd', 'playerNum', 'playerName', 'sourceTeam', 'destTeam', 'metal', 'energy', 'unitIDCount', 'unitIDs')
		elif cmd == 17:
			cmd = 'memdump'
			...
			playerName = self.players[playerNum] or ''
			return self.write(locals(), 'cmd', 'playerNum', 'playerName', 'shareTeam', 'shareUnits', 'shareMetal', 'shareEnergy')
		elif cmd == 27:
			cmd = 'setshare'
			playerNum, team, metalShareFraction, energyShareFraction = struct.unpack('<BBff', data)
			playerName = self.players[playerNum] or ''
			return self.write(locals(), 'cmd', 'playerNum', 'playerName', 'team', 'metalShareFraction', 'energyShareFraction')
		elif cmd == 28:
			...
			if len(data) == 12: # 0.80 or later
				...
				return self.write(locals(), 'cmd', 'size', 'compressedSize', 'setupText', 'mapChecksum', 'modChecksum', 'randomSeed')
			elif False:
				print __import__('binascii').hexlify(data)
			...
			playerName = self.players[playerNum] or ''
			return self.write(locals(), 'cmd', 'playerNum', 'playerName', 'otherAllyTeam', 'allianceState', 'readableAllianceState')
		elif cmd == 54:
			cmd = 'ccommand'
