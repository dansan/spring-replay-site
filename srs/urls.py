...
sitemaps = {
    "replays": ReplaySitemap,
    "players": PlayerAccountSitemap,
    "hall of fame": HofSitemap,
}

urlpatterns = [
    url(r"^$", index, name="srs/index"),
    url(
        r"^index_replay_range/(?P<range_end>[\d]+)/(?P<game_pref>[\d]+)/$",
        index_replay_range,
    ...
    url(
        r"^maplinks_modal/(?P<gameID>[0-9a-f]+)/$",
        maplinks_modal,
        name="srs/maplinks_modal",
    ),
    url(
        r"^modlinks_modal/(?P<gameID>[0-9a-f]+)/$",
        modlinks_modal,
        name="srs/modlinks_modal",
    ),
    url(
        r"^enginelinks_modal/(?P<gameID>[0-9a-f]+)/$",
        enginelinks_modal,
        name="srs/enginelinks_modal",
    ),
    url(r"^stats_modal/(?P<gameID>[0-9a-f]+)/$", stats_modal, name="srs/stats_modal"),
    url(
        r"^feeds/latest_comments/$",
        SRSLatestCommentFeed(),
        name="srs/feeds/latest_comments",
    ),
    url(r"^feeds/latest/$", LatestUploadsFeed(), name="srs/feeds/latest"),
    url(r"^feeds/game/(?P<game>[\w ]+)/$", GameFeed(), name="srs/feeds/game"),
    url(
        r"^feeds/uploader/(?P<username>[\w\ .:()\[\]-]+)/$",
        UploaderFeed(),
        name="srs/feeds/uploader",
    ),
    ...
    url(
        r"^ts_history_graph/(?P<game_abbr>[A-Z0-9]+)/(?P<accountid>[\d]+)/(?P<match_type>[1TFGL])/$",
        ts_history_graph,
        name="srs/ts_history_graph",
    ),
    url(
        r"^ts_history_modal/(?P<game_abbr>[A-Z0-9]+)/(?P<accountid>[\d]+)/(?P<match_type>[1TFGL])/$",
        ratinghistorygraph_modal,
        name="srs/ratinghistorygraph_modal",
    ),
    url(r"^team_stat_div/(?P<ts_id>[\d-]+)/$", team_stat_div, name="srs/team_stat_div"),
    url(
        r"^gamerelease/(?P<gameid>[\d-]+)/$",
        gamerelease_modal,
        name="srs/gamerelease_modal",
    ),
    url(
        r"^hall_of_fame/(?P<abbreviation>[\w\ .:()\[\]-]+)/$",
        hall_of_fame,
    ...
    url(
        r"^ajax_winloss_tbl_src/(?P<accountid>[\d-]+)/$",
        ajax_winloss_tbl_src,
        name="srs/ajax_winloss_tbl_src",
    ),
    url(
        r"^ajax_playerreplays_tbl_src/(?P<accountid>[\d-]+)/$",
        ajax_playerreplays_tbl_src,
