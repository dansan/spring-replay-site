...
urlpatterns = patterns('',
    ...
    url(r'^ts_history_graph/(?P<game_abbr>[A-Z,0-9]+)/(?P<accountid>[\d]+)/(?P<match_type>[1,T,F,G,L])/$', 'srs.views.ts_history_graph', name="ts_history_graph"),
    url(r'^ts_history_modal/(?P<game_abbr>[A-Z,0-9]+)/(?P<accountid>[\d]+)/(?P<match_type>[1,T,F,G,L])/$', 'srs.ajax_views.ratinghistorygraph_modal', name="ratinghistorygraph_modal"),
