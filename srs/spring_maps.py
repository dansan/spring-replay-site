...
#         "filename" : self.mapname,
    ...
    def make_home_thumb(self):
        ...
        self.thumb = self.mapname+"_home.jpg"
    ...
    def create_map_with_boxes(self, replay):
        ...
        players = Player.objects.filter(replay=replay, spectator=False, startposx__isnull=False, startposz__isnull=False)
        if players.exists():
            #
            ...
            #
            for startpos in [(float(pair.split(",")[0]), float(pair.split(",")[1])) for pair in replay.map_info.startpos.split("|")]:
